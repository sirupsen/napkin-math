...
#[inline(never)]
#[no_mangle]
fn memory_read_sequential_single_thread_non_vectorized(vec: &mut [Int]) -> u64 {
    // This prevents using SIMD, somehow the combination of len-1
    // and the array. I can't disable compiling against SIMD easily because some
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    println!("Size in elements: {}", size_in_elements);
    ...
    for core in core_ids.iter() {
        ...
        thread::spawn(move || {
            ...
            let mut range =
                (slice_size as u64 * core.id as u64)..(slice_size as u64 * (core.id as u64 + 1));
            if core.id == last_id {
